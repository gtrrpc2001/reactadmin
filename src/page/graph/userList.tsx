...
export const UserList = ({ nameList, handler, id, width, height }: Props) => {
  const selectedColor = (eq: string, box = false) =>
    `${id == eq ? "#5388F7" : box ? "black" : "#c3c1c1"}`;
  ...
  const itemes = () => {
    try {
      return nameList?.map((name, index) => {
        ...
        return (
          <ListItem className="listItem" id={eq} key={eq} onClick={(e) => handler(e)}>
...
export const TimeList = ({ timeList, handler, id, width, height }: Props) => {
  const selectedColor = (time: string, box = false) =>
    `${id == time ? "#5388F7" : box ? "black" : "#c3c1c1"}`;
              ...
              <ListItemIcon>
                <Box
                  sx={{
                    ...
                    height: 30,
                    borderRadius: 3,
                    bgcolor: selectedColor(value, true),
                    display: "flex",
                    justifyContent: "center",
                    alignItems: "center",
                    ":hover": { cursor: "default" },
